...
class _ApplyState extends State<Apply> with TickerProviderStateMixin {
  ...
  Future _fetch({page = 1}) async {
    return Http.post(API.applyList, data: {'account_token': _token, 'page': page}).then((result){
      if(result['code'] == 1){
        setState(() {
          ...
          Expanded(
            ...
            child: ClipOval(
              child: new AspectRatio(
                aspectRatio: 1,
                child: (list[index]['avatar'] != null && list[index]['avatar'] != '') ? new CachedNetworkImage(imageUrl: API.host + list[index]['avatar'], fit: BoxFit.cover) : Image.asset('assets/images/avatar.jpg', fit: BoxFit.cover),
              )
            ),
          ),
          Expanded(
            ...
            child: Column(
              children: <Widget>[
                ListTile(
                  title: Text(list[index]['truename'] + '(' + list[index]['mobile'] + ')', maxLines: 1, overflow: TextOverflow.ellipsis,),
                  subtitle: new Padding(
                    ...
                    child: new Row(
                      ...
                      crossAxisAlignment: CrossAxisAlignment.center,
                      children: <Widget>[
                        Text(list[index]['avtive']),
                        list[index]['status'] == 1 ? Text('已审核', style: TextStyle(color: Colors.green),) : Text('待审核', style: TextStyle(color: Colors.red),),
                      ],
                    ),
                  ),
                  onTap: (){},
  ...
  Widget build(BuildContext context){
    return MaterialApp(
      ...
      home: Scaffold(
        appBar: AppBar(
          elevation: 0,
          title: Text('邀请好友', style: TextStyle(color: Colors.white)),
