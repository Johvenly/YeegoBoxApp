...
class BDetail extends StatefulWidget{
  final int id;
  BDetail({Key key, this.id}) : super(key: key);

  State<StatefulWidget> createState() => new BDetailState();
}

class BDetailState extends State<BDetail> with TickerProviderStateMixin {
  //初始化页面数据
  Map pageRowData;
  //数据控制字段
  bool _loaded = false;
  ...
  Future<dynamic> initData() async {
    ...
    await Http.post(API.getBrowseDetail, data: {'account_token': _token, 'id': widget.id}).then((result) async{
      if(result['code'] == 1){
        setState(() {
                  ...
                  new Expanded(
                    child: new FlatButton(
                      ...
                      onPressed: () {
                        ...
                        Http.post(API.checkBrowse, data: {'account_token': _token, 'id': widget.id, 'shopname': _shopnameController.text}).then((result){
                            if(result['code'] == 1){
                  ...
                  new Expanded(
                    child: new FlatButton(
                      ...
                      onPressed: () {
                        ...
                        Http.post(API.cancelBrowse, data: {'account_token': _token, 'id': widget.id}).then((result){
                            if(result['code'] == 1){
                              Fluttertoast.showToast(msg: result['msg'], gravity: ToastGravity.CENTER).then((_) async{
                                setState(() {
